...
const AddCouponForm = () => {
    const { isAddCouponForm, setIsAddCouponForm } = useContext(CouponContext);
    ...
    const handleSubmit = async (e) => {
        ...
        if (code && discount && expiry && description && minorder) {
            ...
            const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/coupons/add`, {
    ...
    return (
        <div className={`bg-[#7e7e7f53] w-full fixed top-0 left-0 h-screen overflow-y-auto flex items-center justify-center p-4 ${isAddCouponForm ? 'visible' : 'hidden'}`}>


            <div on className='md:w-[848px] w-[90vw] max-h-[90vh] overflow-y-auto rounded-[16px] drop-shadow-sm bg-white z-10 p-4 md:p-10'>

                <h1 className='font-bold text-2xl'>Add new coupon</h1>
                <form onSubmit={handleSubmit} className='w-full flex flex-col gap-2 md:gap-4 mt-5'>
                    ...
                    <div className='flex w-full gap-2 items-center '>
                        ...
                        <button onClick={() => setIsAddCouponForm(false)} type='button' className='h-[59px] border-2 border-black rounded-[10px]  text-xl font-semibold text-black flex-1 s'>
                            close
                        </button>
                    </div>
